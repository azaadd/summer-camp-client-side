...
const AllUsers = () => {
    ...
    const handleMakeAdmin = user => {
        fetch(`https://b7a12-summer-camp-server-side-beta.vercel.app/usersInfo/admin/${user._id}`, {
            method: 'PATCH'
        })
        .then(res => res.json())
        .then(data => {
    ...
    const handleDelete = user => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                fetch(`https://b7a12-summer-camp-server-side-beta.vercel.app/usersInfo/${user._id}`, {
                    method: 'DELETE'
                })
                    .then(res => res.json())
                    .then(data => {
